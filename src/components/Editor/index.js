...
class Editor extends React.Component {
  ...
  handleAddCoverToFB = async (file) => {
    ...
    const decideCoverLocation = this.postId
      ? uniqueStorageId
      : this.uniqueStorageId

    const imgRef = this.postId
      ? storage.ref(
          `${postType}/${decideCoverLocation}/images/cover/${file.name}`,
        )
      : storage
          .ref(`${postType}/${decideCoverLocation}/images`)
          .child(`cover/${file.name}`)
    ...
    try {
      ...
      content.uniqueStorageId = content.uniqueStorageId
        ? content.uniqueStorageId
        : this.uniqueStorageId
  ...
  handleUploadContentEditorImage = async (file) => {
    ...
    */
    ...
    const imgRef = this.postId
      ? storage.ref(
          `${postType}/${decideCoverLocation}/images/content/${file.name}`,
        )
      : storage
          .ref(`${postType}/${decideCoverLocation}/images/content`)
          .child(`${file.name}`)
  ...
  fetchPost = () => {
    const postRef = this.props.firebase.db.ref(
  ...
  render() {
    ...
    const disabled =
      title === '' || description === '' || cover === '' || editorContent === ''
    ...
    return (
      <>
        <HeaderContainer
          cover={url}
          height="50vh"
          title={title ? title : 'Create your story'}
        />
        <Container
          maxWidth="lg"
