...
export function DotsEditor({ firebase }) {
  ...
  const onDeleteDotHandler = (id) => {
    ...
    if (confirm) {
      const postRef = firebase.db.ref(`${searchValues.post}/${searchValues.key}/dotsCoord/${id}`);

      postRef
        .remove()
        .then(() => {
          emptyDotsValues();
          handleOpen("success", `${searchValues.post}/${searchValues.key}/dotsCoord/${id} removed successfully`);
        })
  ...
  useEffect(() => {
    const postRef = firebase.db.ref(searchValues.post);
    const postRefListener = postRef.on("value", (snap) => {
      let newPosts = [];
    ...
    return () => postRef.off("value", postRefListener);
      ...
      <Container maxWidth="lg" style={{ paddingTop: "5rem", paddingBottom: "5rem" }}>
        <Grid container justify="space-between" alignItems="flex-start">
          ...
          <Grid container item lg={5} direction="column" justify="center">
            <Typography component="h4" color="secondary" style={{ marginBottom: ".5rem" }}>
